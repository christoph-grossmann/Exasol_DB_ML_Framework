 ...
 */
CREATE OR REPLACE PYTHON3 SET SCRIPT ML.sklearn_ensemble_RandomForestClassifier_train(...) RETURNS /*name*/ VARCHAR(64) AS
import pandas as pd
import numpy as np
import json
...
def run(ctx):
  ...
  if ctx[1] != '' and ctx[1] != None:
  	...
  	settings = []
  helper.TrainingStepInit(ctx, 'classification', range(2, exa.meta.input_column_count - 1), [ exa.meta.input_column_count - 1 ], 'ALL')
  ...
  helper.DumpToBucketFS(helper.map_out, filename + '_map_out.json')
  return name
